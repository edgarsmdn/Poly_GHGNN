...
class GHGNN(nn.Module):
    def __init__(self, v_in, e_in, u_in, hidden_dim, ext_in):
        ...
        # A separate first layer for the polymer
        self.graphnet1solu = gnn.MetaLayer(EdgeModel(v_in, e_in, u_in+ext_in, 
                                                     hidden_dim),
                                      NodeModel(v_in, u_in+ext_in, hidden_dim), 
                                      GlobalModel(u_in+ext_in, hidden_dim))
    ...
    def forward(self, solvent, solute, T):
        ...
        # -- Extra feature(s) for the polymer
        ext21 = solute.ext1.reshape(-1,1)
        try:
            ext22 = solute.ext2.reshape(-1,1)
        except:
            ext22 = None
            pass
        
        u1 = torch.cat((ap1,bp1,topopsa1), axis=1) # Molecular descriptors solvent
        if ext22 is None: # Molecular descriptors solute
            u2 = torch.cat((ap2,bp2,topopsa2,ext21), axis=1) 
        else:
            u2 = torch.cat((ap2,bp2,topopsa2,ext21, ext22), axis=1)
        ...
        if solvent.edge_attr.shape[0] == 0 or solute.edge_attr.shape[0] == 0:
            ...
            solute = cat_dummy_graph(solute, solute_bool=True, ext22=ext22)
            u1_dummy = torch.tensor([1,1,1]).reshape(1,-1)
            if ext22 is None:
                u2_dummy = torch.tensor([1,1,1,1]).reshape(1,-1)
            else:
                u2_dummy = torch.tensor([1,1,1,1,1]).reshape(1,-1)
            if torch.cuda.is_available():
