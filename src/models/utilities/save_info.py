...
def save_train_traj(path, df_model_training, method_name, valid=True):
    if not os.path.exists(path):
        os.makedirs(path)
    df_model_training.to_csv(path+'/training_traj_'+method_name+'.csv')
    ...
    plt.legend()
    plt.close(fig)
    fig.savefig(path+'/training_traj_'+method_name+'.png')
