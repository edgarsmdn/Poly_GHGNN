...
def sys2graph(df, mol_column_1, mol_column_2, target, extra1, extra2=None,
              y_scaler=None, single_system=False):
    '''
    ...
    if single_system:
        ...
        ys   = df[target].tolist()
        exts1  = df[extra1].tolist()
        if extra2 is not None:
            exts2  = df[extra2].tolist()
    
    graphs_solvent, graphs_solute = [], []
    if extra2 is None:
        zz = zip(ys, solvents, solutes, exts1)
    else:
        zz = zip(ys, solvents, solutes, exts1, exts2)
    for tup in tqdm(zz, total=len(ys)):
        
        if extra2 is None:
            y, solv, solu, ext1 = tup
        else:
            y, solv, solu, ext1, ext2 = tup
...
def get_dummy_graph(solute_bool, ext22):
    ...
    if solute_bool and ext22 is None:
        ap, bp, topopsa, hb, inter_hb, y, ext1 = [torch.tensor([1])]*7
        graph_dummy = Data(x=nodes_info_solv, edge_index=edges_indx_solv, edge_attr=edges_info_solv,
                           ap=ap, bp=bp, topopsa=topopsa, hb=hb, inter_hb=inter_hb, y=y, ext1=ext1)
    elif solute_bool and ext22 is not None:
        ap, bp, topopsa, hb, inter_hb, y, ext1, ext2 = [torch.tensor([1])]*8
        graph_dummy = Data(x=nodes_info_solv, edge_index=edges_indx_solv, edge_attr=edges_info_solv,
                           ap=ap, bp=bp, topopsa=topopsa, hb=hb, inter_hb=inter_hb, y=y, ext1=ext1, ext2=ext2)
    else:
        ap, bp, topopsa, hb, inter_hb, y = [torch.tensor([1])]*6
        graph_dummy = Data(x=nodes_info_solv, edge_index=edges_indx_solv, edge_attr=edges_info_solv,
...
def cat_dummy_graph(batch, solute_bool=False, ext22=None):
    dummy_graph = get_dummy_graph(solute_bool, ext22)
    ...
    if solute_bool and ext22 is None:
        batch.ext1 = torch.cat((batch.ext1, dummy_graph.ext1))
    elif solute_bool and ext22 is not None:
        batch.ext1 = torch.cat((batch.ext1, dummy_graph.ext1))
        batch.ext2 = torch.cat((batch.ext2, dummy_graph.ext2))
